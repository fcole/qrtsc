...
{
    int data_size, err, byteswritten = 0;
	Bytef data_tag[2];
    int buf_size = 1024, i;
...
{
    int nBytes = 0, data_size, err, byteswritten = 0;
	Bytef data_tag[2];
    int buf_size = 1024;
    mat_uint8_t   buf[1024], pad[8] = {0,};
    ...
    if ( matvar->compression == COMPRESSION_NONE ) {
        ...
        switch ( matvar->class_type ) {
            ...
            case MAT_C_STRUCT:
            {
                ...
                for ( i = 0; i < nfields; i++ ) {
                    fieldnames[i] = fields[i]->name;
                    if ( (int)(strlen(fieldnames[i])) > maxlen )
                        maxlen = strlen(fieldnames[i]);
                }
                maxlen++;
                fieldname_size = maxlen;
